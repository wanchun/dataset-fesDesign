...
import { IPublicTypeNpmInfo } from './npm';
import { IPublicTypePropConfig } from './prop-config';
import { IPublicTypeCompositeValue } from './value-type';
import { IPublicTypeComponentSchema, IPublicTypeFieldConfig, IPublicTypeIcon, IPublicTypeNodeSchema, IPublicTypeProjectSchema } from '.';
/**
 * 可用片段
 *
...
export interface IPublicTypeTransformedComponentMetadata extends IPublicTypeComponentMetadata {
    configure: IPublicTypeConfigure & {
        combined?: IPublicTypeFieldConfig[];
    };
}
/**
 * 嵌套控制函数
...
export interface IPublicTypeEventParam {
    name: string;
    type: string;
}
export type IPublicTypeConfigureSupportEvent = string | {
    name: string;
    params?: IPublicTypeEventParam[];
    description?: string;
};
...
export declare function isActionContentObject(obj: any): obj is IPublicTypeActionContent;
